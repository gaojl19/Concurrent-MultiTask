...
class EMMultiHeadPolicy(nn.Module):
    ...
    def train_forward(self, obs, acs, idx, criterion, log, use_log_prob=False):
        '''
        ...
        if use_log_prob:
            ...
            for i in range(len(mean_list)):
                ...
                # print((criterion(action, acs).sum(dim=1)/acs.shape[1]).grad)
